...
func TestEventsEndpointStreamingSnapshotReflectsUpdatedRateLimits(t *testing.T) {
	...
	router := NewRouterWithPubSub(zerolog.Nop(), service, &config.Config{WorkspaceRoot: t.TempDir(), Host: "127.0.0.1", APIToken: ""}, pubsub, nil, nil, nil, nil, nil, nil)
...
func TestEventsEndpointStreamsPubSubEvent(t *testing.T) {
	pubsub := observability.NewPubSub()
	router := NewRouterWithPubSub(zerolog.Nop(), orchestrator.NewService(), &config.Config{WorkspaceRoot: t.TempDir(), Host: "127.0.0.1", APIToken: ""}, pubsub, nil, nil, nil, nil, nil, nil)
