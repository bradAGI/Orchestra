...
func (c *Client) FetchCandidateIssues(ctx context.Context, activeStates []string) ([]tracker.Issue, error) {
	...
	query := "SELECT id, identifier, title, description, state, assignee_id, project_id, priority, branch_name, url, labels, blocked_by, provider, disabled_tools, created_at, updated_at, base_sha, feedback FROM issues WHERE LOWER(TRIM(state)) IN ("
...
func (c *Client) FetchIssuesByIDs(ctx context.Context, issueIDs []string) ([]tracker.Issue, error) {
	...
	query := "SELECT id, identifier, title, description, state, assignee_id, project_id, priority, branch_name, url, labels, blocked_by, provider, disabled_tools, created_at, updated_at, base_sha, feedback FROM issues WHERE id IN ("
...
func (c *Client) FetchIssues(ctx context.Context, filter tracker.IssueFilter) ([]tracker.Issue, error) {
	query := "SELECT id, identifier, title, description, state, assignee_id, project_id, priority, branch_name, url, labels, blocked_by, provider, disabled_tools, created_at, updated_at, base_sha, feedback FROM issues"
	var where []string
	var args []any
...
func (c *Client) SearchIssues(ctx context.Context, query string) ([]tracker.Issue, error) {
	...
	sqlQuery := "SELECT id, identifier, title, description, state, assignee_id, project_id, priority, branch_name, url, labels, blocked_by, provider, disabled_tools, created_at, updated_at, base_sha, feedback FROM issues WHERE title LIKE ? OR identifier LIKE ? OR id LIKE ?;"
	pattern := "%" + query + "%"
...
func (c *Client) FetchIssueByIdentifier(ctx context.Context, identifier string) (*tracker.Issue, error) {
	query := "SELECT id, identifier, title, description, state, assignee_id, project_id, priority, branch_name, url, labels, blocked_by, provider, disabled_tools, created_at, updated_at, base_sha, feedback FROM issues WHERE id = ? OR identifier = ?;"
...
func (c *Client) queryIssues(ctx context.Context, query string, args ...any) ([]tracker.Issue, error) {
	...
	for rows.Next() {
		...
		var title, description, assigneeID, projectID, branchName, url, labelsRaw, blockedByRaw, provider, disabledToolsRaw, createdAt, updatedAt, baseSHA, feedback sql.NullString

		if err := rows.Scan(
			&issue.ID, &issue.Identifier, &title, &description, &issue.State, &assigneeID, &projectID, &issue.Priority,
			&branchName, &url, &labelsRaw, &blockedByRaw, &provider, &disabledToolsRaw, &createdAt, &updatedAt, &baseSHA, &feedback,
		); err != nil {
