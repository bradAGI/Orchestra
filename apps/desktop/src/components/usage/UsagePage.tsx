...
import type { BackendConfig, ProviderRateLimits } from '@/lib/orchestra-client'
...
import type { UsageScope, UsageRange } from '@/lib/orchestra-client'
...
function RateLimitTileBody({ limits }: { limits: ProviderRateLimits | null }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {limits.error && (
        <div className="flex items-start gap-1.5 text-[10.5px] text-yellow-500">
          <AlertTriangle size={10} className="mt-0.5 shrink-0" />
          <span>{limits.error}</span>
        </div>
      )}
    </div>
  )
