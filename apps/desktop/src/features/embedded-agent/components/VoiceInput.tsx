...
export function VoiceInput({ onTranscription, disabled }: VoiceInputProps) {
  ...
  const clientRef = useRef<import('@/workers/whisper-client').WhisperClient | null>(null)

  const getClient = useCallback(async (): Promise<import('@/workers/whisper-client').WhisperClient> => {
    if (!clientRef.current) {
      const { getWhisperClient } = await import('@/workers/whisper-client')
