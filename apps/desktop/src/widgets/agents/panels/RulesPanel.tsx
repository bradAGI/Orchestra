...
const PLACEHOLDER = `# Rule content
...
export function RulesPanel({ items, saving, onSave, onDelete }: FileListPanelProps) {
  ...
  const handleAdd = () => {
    const name = window.prompt('Rule filename (e.g. no-console.md):')
    if (!name) return
    onSave(name, '')
  }
    ...
    <div className="flex h-full">
      ...
      <div className="w-[200px] flex flex-col border-r border-border/30 shrink-0">
        <div className="px-3 pt-3 pb-2 shrink-0">
          ...
          <p className="text-[10px] text-muted-foreground/40 mt-0.5">Path-scoped instructions in .claude/rules/</p>
        </div>
