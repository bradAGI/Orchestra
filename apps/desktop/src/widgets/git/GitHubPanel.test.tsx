...
import type { GitHubIssue, GitHubPR, BackendConfig } from '@/lib/orchestra-client'
...
describe('GitHubPanel', () => {
  ...
  it('shows create issue form', async () => {
    ...
    // Click the + button in issues tab area (it's a Plus icon button)
    const buttons = screen.getAllByRole('button')
    // Find the + toggle button — it's near the filter buttons
    const plusButton = buttons.find(
      (b) => b.querySelector('svg') && b.closest('.flex.items-center.gap-1.mb-2')
    )
    if (plusButton) {
      fireEvent.click(plusButton)
    } else {
      // Fallback: get all buttons and click the one that toggles the form
      // The Plus button is after the filter row
      const allBtns = screen.getAllByRole('button')
      for (const btn of allBtns) {
        if (btn.querySelector('[class*="lucide"]') || btn.innerHTML.includes('svg')) {
          // Check if clicking reveals the form
        }
      }
    }
  ...
  it('renders issue state badges (open/closed)', async () => {
    ...
    expect(issue1Icon?.classList.toString()).toContain('text-green-400')
    ...
    expect(issue2Icon?.classList.toString()).toContain('text-red-400')
  })
})
